...
func TestCheckShellDepth_Unix(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Skipping Unix shell depth test on Windows")
	}
...
// Test shell depth detection on Windows
func TestCheckShellDepth_Windows(t *testing.T) {
	if runtime.GOOS != "windows" {
		t.Skip("Skipping Windows shell depth test on non-Windows")
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedContains: []string{
				...
				"Close and reopen your terminal from time to time",
			},
		},
		{
			name:            "warnings_disabled",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create test options
			opts := &Options{
				DepthWarningThreshold: 15, // Irrelevant for Windows
				DisableDepthWarnings:  tt.disableWarnings,
			}
...
func TestShellDepthIntegration(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("Skipping Unix integration test on Windows")
	}
