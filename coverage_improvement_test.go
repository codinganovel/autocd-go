...
// Test getScriptExtensionForShell - currently 0% coverage
func TestGetScriptExtensionForShell(t *testing.T) {
	tests := []struct {
		...
		{ShellCmd, ".bat"},
		{ShellPowerShell, ".ps1"},
		{ShellPowerShellCore, ".ps1"},
		{ShellUnknown, ".sh"},
		{ShellType(999), ".sh"}, // Unknown shell type
	}
...
// Test isValidWindowsPath - currently 0% coverage
func TestIsValidWindowsPath(t *testing.T) {
	tests := []struct {
		...
		{`C:\invalid?path`, false},
		{`C:\invalid*path`, false},
		{`C:\path\with\x00null`, false},
		{`C:\path\with\x1fcontrol`, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.path, func(t *testing.T) {
			...
			if got != tt.valid {
				t.Errorf("isValidWindowsPath(%q) = %v, want %v", tt.path, got, tt.valid)
			}
		})
	}
}
...
// Test detectWindowsShell - currently 0% coverage
func TestDetectWindowsShell(t *testing.T) {
	...
	if (shell.Type == ShellPowerShell || shell.Type == ShellPowerShellCore) && shell.ScriptExt != ".ps1" {
		t.Error("PowerShell should have .ps1 extension")
	}
}
...
func TestSetExecutablePermissions_ErrorCase(t *testing.T) {
	if runtime.GOOS == "windows" {
		// Windows always returns nil
		err := SetExecutablePermissions("/any/path")
		if err != nil {
			t.Error("SetExecutablePermissions should return nil on Windows")
		}
		return
	}
...
// Test classifyUnixShell with all shell types
func TestClassifyUnixShell_AllTypes(t *testing.T) {
	tests := []struct {
		shellPath string
		expected  ShellType
	}{
	...
	for _, tt := range tests {
		t.Run(tt.shellPath, func(t *testing.T) {
			got := classifyUnixShell(tt.shellPath)
			if got != tt.expected {
				t.Errorf("classifyUnixShell(%q) = %v, want %v", tt.shellPath, got, tt.expected)
			}
		})
	}
}
...
func TestIsRecoverable_AllErrorTypes(t *testing.T) {
	tests := []struct {
		...
		{ErrorPlatformUnsupported, false},
